...
const DB_CLASSNAME_FOR_METHOD = {
    GET: 'favorite-pos_get',
    POST: 'favorite-pos_post',
    PUT: 'favorite-pos_put',
    PATCH: 'favorite-pos_patch',
    DELETE: 'favorite-pos_delete',
    HEAD: 'favorite-pos_head',
    OPTIONS: 'favorite-pos_options',
};
        ...
        if (ID_USER && ID_USER in data) {
            ...
            if (newItem) {
                ...
                writeJsonFileAsync(DB_USERS_LIST_PATH, data, (writeErr) => {
                    ...
                    res.render("favorites_list.hbs", { dataList: dataListWithId(data[ID_USER]) }, (err, html) => {
                        if (err) {
                            return res.status(500).json({ error: 'Ошибка рендеринга' });
                        }
                        res.json({ html, list: data[ID_USER] });
                    });
            ...
            writeJsonFileAsync(DB_USERS_LIST_PATH, newData, (writeErr) => {
                ...
                res.render("favorites_list.hbs", { idUser: newId, dataList: dataListWithId(data[ID_USER]) }, (err, html) => {
                    if (err) {
                        return res.status(500).json({ error: 'Ошибка рендеринга' });
                    }
                    res.json({ id: newId, html, list: data[ID_USER]});
                });
