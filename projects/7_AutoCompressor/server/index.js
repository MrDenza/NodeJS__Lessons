...
let getNumWord = (num, word1 = 'поток', word2 = 'потока', word5 = "потоков") => {
    const dd = num % 100;
...
async function main () {
    ...
    log('success', 'MODE-WORK', `Выбрано для работы ${processingMode} ${getNumWord(processingMode)} процессора (параллельных задач).`);
    ...
    console.log();
    log('warn', 'COMPRESS-TIMER', `Обработка файлов заняла ${duration} мс. Параллельно выполнялось: ${processingMode} ${getNumWord(processingMode)}.`);
